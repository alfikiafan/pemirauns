<!-- Navbar -->
<nav class="navbar navbar-main navbar-expand-lg p-0 mx-4 shadow-none border-radius-xl" id="navbarBlur"
    navbar-scroll="true">
    <div class="container-fluid py-1 px-3">
        <nav aria-label="breadcrumb" style="padding-left: 250px;">
            ...
            <ul class="navbar-nav justify-content-end">
                <li class="nav-item d-flex align-items-center px-3">
                    <a href="#" class="nav-link text-body p-0">
                        ...
                        <span class="d-sm-inline d-none text-muted">Admin Fakultas
                            ({{ $userRole->faculty ?? 'Nama Fakultas Tidak Tersedia' }})</span>
                        @elseif($userRole->role->name == 'superadmin')
                        <span class="d-sm-inline d-none text-muted">Super Admin</span>
                        @endif
                        @else
                        <span class="d-sm-inline d-none text-muted">Pemilih -
                            {{ $user->faculty ?? 'Nama Fakultas Tidak Tersedia' }}</span>
                        @endif
                    </a>
                </li>
                ...
                <li class="nav-item d-flex align-items-center">
                    <form class="mb-0" action="{{ route('logout') }}" method="POST" style="display: inline;">
                        @csrf
                        <button type="submit" class="nav-link text-body font-weight-bold px-0 py-0"
